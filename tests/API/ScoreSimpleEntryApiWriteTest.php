...
final class ScoreSimpleEntryApiWriteTest extends DbWebTestCase
{
    public function testOwnerCreatesScoreSimpleEntryViaApi(): void
    {
        $client = static::createClient();
        $em = static::getContainer()->get(EntityManagerInterface::class);

        $owner = new User();
        $owner->setEmail('api-entry-owner@test.local');
        $owner->setPassword('dummy');
        $owner->setCreatedAt(new \DateTimeImmutable());

        $group = new GameGroup();
        $group->setName('Entry Group');
        ...
        $client->request(
            'POST',
            '/api/groups/' . $group->getId() . '/sessions/' . $session->getId() . '/entries/score-simple',
            server: ['CONTENT_TYPE' => 'application/json'],
            content: json_encode([
                'label' => 'Manche 1',
                'scores' => [
                    ['participantName' => 'Mathias', 'score' => 42],
                    ['participantName' => 'Lucas', 'score' => 38],
                ],
            ], JSON_THROW_ON_ERROR)
        );
        ...
        self::assertSame('score_simple', $payload['data']['type'] ?? null);
        ...
        self::assertSame(EntryType::SCORE_SIMPLE, $entry->getType());
        self::assertSame('Manche 1', $entry->getLabel());
        self::assertCount(2, $entry->getScores());
    }

    public function testAnonymousGets401OnCreateScoreSimpleEntryApi(): void
    {
        $client = static::createClient();
        $client->request(
            'POST',
            '/api/groups/1/sessions/1/entries/score-simple',
            server: ['CONTENT_TYPE' => 'application/json'],
            content: json_encode([
                'scores' => [
                    ['participantName' => 'Mathias', 'score' => 42],
    ...
    public function testMemberGets403OnCreateScoreSimpleEntryApi(): void
    {
        $client = static::createClient();
        $em = static::getContainer()->get(EntityManagerInterface::class);

        $owner = new User();
        $owner->setEmail('api-entry-owner-2@test.local');
        $owner->setPassword('dummy');
        $owner->setCreatedAt(new \DateTimeImmutable());

        $memberUser = new User();
        $memberUser->setEmail('api-entry-member@test.local');
        $memberUser->setPassword('dummy');
        $memberUser->setCreatedAt(new \DateTimeImmutable());

        $group = new GameGroup();
        $group->setName('Forbidden Entry Group');
        $group->setCreatedBy($owner);
        $group->setCreatedAt(new \DateTimeImmutable());
    ...
    public function testMissingScoresGets422OnCreateScoreSimpleEntryApi(): void
    {
        $client = static::createClient();
        $em = static::getContainer()->get(EntityManagerInterface::class);

        $owner = new User();
        $owner->setEmail('api-entry-validation-1@test.local');
        $owner->setPassword('dummy');
        $owner->setCreatedAt(new \DateTimeImmutable());

        $group = new GameGroup();
        $group->setName('Entry Validation Group 1');
        $group->setCreatedBy($owner);
        $group->setCreatedAt(new \DateTimeImmutable());
        ...
        $payload = json_decode((string) $client->getResponse()->getContent(), true);
        self::assertSame('scores_required', $payload['error']['code'] ?? null);
    }

    public function testEmptyScoresGets422OnCreateScoreSimpleEntryApi(): void
    {
        $client = static::createClient();
        $em = static::getContainer()->get(EntityManagerInterface::class);

        $owner = new User();
        $owner->setEmail('api-entry-validation-2@test.local');
        $owner->setPassword('dummy');
        $owner->setCreatedAt(new \DateTimeImmutable());

        $group = new GameGroup();
        $group->setName('Entry Validation Group 2');
        $group->setCreatedBy($owner);
        $group->setCreatedAt(new \DateTimeImmutable());
        ...
        $client->request(
            ...
            '/api/groups/' . $group->getId() . '/sessions/' . $session->getId() . '/entries/score-simple',
            server: ['CONTENT_TYPE' => 'application/json'],
            content: json_encode(['scores' => []], JSON_THROW_ON_ERROR)
        );

        self::assertResponseStatusCodeSame(422);

        $payload = json_decode((string) $client->getResponse()->getContent(), true);
        self::assertSame('scores_empty', $payload['error']['code'] ?? null);
    }

    public function testInvalidJsonGets400OnCreateScoreSimpleEntryApi(): void
    {
        $client = static::createClient();
        $em = static::getContainer()->get(EntityManagerInterface::class);

        $owner = new User();
        $owner->setEmail('api-entry-invalid-json@test.local');
        $owner->setPassword('dummy');
        $owner->setCreatedAt(new \DateTimeImmutable());
    ...
    public function testSessionFromDifferentGroupGets422OnCreateScoreSimpleEntryApi(): void
    {
        $client = static::createClient();
        $em = static::getContainer()->get(EntityManagerInterface::class);
        ...
        // Owner 1 tries to create an entry in Group 1 using a session from Group 2
        $client->loginUser($owner1);
        $client->request(
            'POST',
            '/api/groups/' . $group1->getId() . '/sessions/' . $session->getId() . '/entries/score-simple',
            server: ['CONTENT_TYPE' => 'application/json'],
            content: json_encode([
                'scores' => [
                    ['participantName' => 'Mathias', 'score' => 42],
    ...
    public function testInvalidScoreValueGets422OnCreateScoreSimpleEntryApi(): void
    {
        $client = static::createClient();
        $em = static::getContainer()->get(EntityManagerInterface::class);

        $owner = new User();
        $owner->setEmail('api-entry-invalid-score@test.local');
        $owner->setPassword('dummy');
        $owner->setCreatedAt(new \DateTimeImmutable());

        $group = new GameGroup();
        $group->setName('Entry Validation Group 3');
        $group->setCreatedBy($owner);
        $group->setCreatedAt(new \DateTimeImmutable());
        ...
        $client->request(
            ...
            content: json_encode([
                'scores' => [
                    ['participantName' => 'Mathias', 'score' => 'not_a_number'],
                ],
            ], JSON_THROW_ON_ERROR)
        );
        ...
        $payload = json_decode((string) $client->getResponse()->getContent(), true);
        self::assertSame('score_must_be_numeric', $payload['error']['code'] ?? null);
    }
}
